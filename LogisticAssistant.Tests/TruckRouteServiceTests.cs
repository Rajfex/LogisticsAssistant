...
namespace LogisticAssistant.Tests
{
    public class TruckRouteServiceTests
    {
        ...
        public async Task CreateTruckRouteAsync_ShouldReturnFasle_WhenVmaxIsZero()
        {
            var context = GetDbContext();
            var httpContextAccessor = GetHttpContextAccessor();
            context.Trucks.Add(new Truck { Id = 1, LicensePlate = "ABC123", Vmax = 0, DriverBreak = 15 });
        ...
        public async Task CreateTruckRouteAsync_ShouldReturnFalse_WhenBreakFrequencyIsZero()
        {
            var context = GetDbContext();
            var httpContextAccessor = GetHttpContextAccessor();
            context.Trucks.Add(new Truck { Id = 1, LicensePlate = "ABC123", Vmax = 100, DriverBreak = 15 });
